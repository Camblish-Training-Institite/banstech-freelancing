        ...
        <div>
            ...
            <div class="flex space-x-8 mt-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800">${{ 70 }}</p> {{-- number_format($project->milestones->where('status', 'in_progress')->sum('amount'), 2) --}}
                </div>
                <div>
                    <p class="text-gray-500">Released</p>
                    <p class="text-2xl font-bold text-gray-800">${{ 400 }}</p> {{-- number_format($project->milestones->where('status', 'released')->sum('amount'), 2) --}}
                </div>
            </div>
        </div>
    ...
    <div class="p-6 border-t border-gray-200">
        ...
        <table class="min-w-full">
            ...
            <tbody class="bg-white">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                            @if($milestone->status != 'released')
                                <a href="#" class="text-indigo-600 hover:text-indigo-900">Release</a>
                            @else
                ...
                @endforeach --}}
                <tr>
                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ '12 August 2025' }}</td>
                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ 'Draw the outline' }}</td> {{-- $milestone->description --}}
                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                        {{-- <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                            @if($milestone->status == 'released') bg-green-100 text-green-800 @else bg-yellow-100 text-yellow-800 @endif">
                            {{ ucfirst(str_replace('_', ' ', $milestone->status)) }}
                    ...
                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">${{ 100 }}</td> {{-- number_format($milestone->amount, 2) --}}
                    <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                        {{-- @if($milestone->status != 'released')
                            <a href="#" class="text-indigo-600 hover:text-indigo-900">Release</a>
                        @else
