...
<div class="container mx-auto px-4 md:px-12">
    <div class="flex w-full justify-start">
        <a href="{{ route('freelancer.projects.list') }}" class="back-link">← Back to Projects</a>
    </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex items-center space-x-2">
                    <div class="flex items-center justify-center rounded-full object-cover overflow-hidden" style="width: 4rem; height:4rem;">
                        <img class="" src="{{$project->job->user->profile? asset('storage/'.$project->job->user->profile->avatar) : "https://ui-avatars.com/api/?name=". $project->job->user->name . "&background=random&size=128" }}" alt="{{ $project->job->user->name }}">
                    </div>
                    <div>
            ...
            <div x-data="{ tab: 'details' }">
                ...
                <div class="py-6">
                    <div x-show="tab === 'details'">
                        @include('Users.Freelancers.projects.tabs.details')
                    </div>
                    <div x-show="tab === 'payments'">
                        @include('Users.Freelancers.projects.tabs.payments')
                    </div>
                    <div x-show="tab === 'files'">
                        @include('Users.Freelancers.projects.tabs.files')
                    </div>
                    <div x-show="tab === 'tasks'">
                        @include('Users.Freelancers.projects.tabs.tasks')
                    </div>
                </div>
            </div>
        </div>
